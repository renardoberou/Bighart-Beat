...
(function (root) {
  ...
  function resolveSynthVoiceSpec(engineId, params) {
    ...
    const acidFilterDecayMax = decaySec * 1.4;
    const acidFilterDecayMin = Math.min(filterAttackSec + 0.0005, acidFilterDecayMax);
    const filterDecaySec = profile.personality === 'acid-bass'
      ? clamp(rawFilterDecaySec, acidFilterDecayMin, acidFilterDecayMax)
      : clamp(rawFilterDecaySec, filterAttackSec + 0.0005, decaySec);
