...
(function (root) {
  ...
  function resolveHihatVoiceSpec(engineId, params, rand, velocityOrAccent) {
    ...
    const engineTailBias = engine === '808' ? 1.15 : (engine === 'reznor' ? 1.20 : (engine === '909' ? 0.90 : 1.0));
    const noiseTailSec = clamp(decaySec * characterTailDamp * (1 + open * 0.08) * velocityTail * engineTailBias, 0.006, 0.70);
    const metalTailSec = clamp(decaySec * characterTailDamp * (0.72 + open * 0.08) * velocityTail * engineTailBias, 0.004, 0.56);
    ...
    const airLowpassHz = clamp(freq * profile.bright * characterAirDamp * (1.25 - open * 0.22) * (1 - softHit * 0.08 + accentedHit * 0.12 + aphexClosedNeedleAccent * 0.03) * (1 + openAphexMetalAir * 0.13), 8500, 18000);
    ...
    const openSplashEngineTail = isReznor ? 1.24 : (isAphex ? 0.88 : (engine === '909' ? 0.72 : 0.95));
    const openSplashEngineHz = isReznor ? 0.72 : (isAphex ? 1.25 : (engine === '909' ? 1.12 : 0.92));
    ...
    const openShimmerHz = clamp(10000 * profile.bright * (1 + open * 0.18) * (1 - softHit * 0.05 + accentedHit * 0.08) * (1 + aphexShimmerEdgeBoost * 0.08) * jitter(rand, instability * 0.4), 6500, 18000);
