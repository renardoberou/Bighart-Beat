...
assert(/function\s+triggerSynthChoke\s*\(\s*t,\s*voiceGain,\s*spec,\s*cleanup\s*\)\s*\{[\s\S]*const\s+previous\s*=\s*synthVoiceState\.gain[\s\S]*previous\.gain[\s\S]*setTargetAtTime\(\.0008,\s*t,\s*spec\.chokeTau\)[\s\S]*previousCleanup\(\s*cleanupAt\s*\)/.test(main), 'synth retrigger chokes/release-ramps and cleanup-retires previous mono voice');
