...
assert(/function\s+synthSynth\s*\(\s*t,\s*v,\s*p,\s*options\s*=\s*\{\s*\}\s*\)\s*\{[\s\S]*SynthVoice\.resolveSynthVoiceSpec\(S\.engine,\s*p\)[\s\S]*routeVoice\(t,\s*6,\s*spec\.stopSec\)[\s\S]*if\s*\(\s*!audition\s*\)\s*\{\s*triggerSynthChoke\(t,\s*voiceGain,\s*spec\);\s*\}/.test(main), 'synthSynth uses selected engine resolver, routeVoice with the synth tail, and mono choke for non-audition playback');
...
assert(/function\s+applySynthGlideFrequency\s*\(\s*frequencyParam,\s*targetHz,\s*t,\s*spec,\s*shouldGlide,\s*previousTargetHz\s*\)\s*\{[\s\S]*frequencyParam\.setValueAtTime\(shouldGlide\s*\?\s*previousTargetHz\s*:\s*targetHz,\s*t\)[\s\S]*if\s*\(shouldGlide\)\s*frequencyParam\.setTargetAtTime\(targetHz,\s*t,\s*spec\.glideSec\)/.test(main), 'shared synth glide helper starts frequency params at previous scaled pitch then targets current scaled pitch');
...
assert(/function\s+previewSynth\s*\(\s*\)\s*\{[\s\S]*initAudio\(\)[\s\S]*triggerCompGate\(t,\s*tr\.id\)[\s\S]*synthSynth\(t,\s*tr\.vol,\s*\{\s*\.\.\.tr\.p,\s*pitch:\s*getStepSynthPitch\(LAST_SYNTH_NOTE_STEP\)\s*\},\s*\{\s*audition:\s*true\s*\}\s*\)/.test(main), 'TEST SYN audition uses selected/last-edited harmonic step pitch, preserves compressor gate, and isolates mono state');
