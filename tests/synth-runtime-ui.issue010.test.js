...
assert(/function\s+synthSynth\s*\(\s*t,\s*v,\s*p\s*\)\s*\{[\s\S]*SynthVoice\.resolveSynthVoiceSpec\(S\.engine,\s*p\)[\s\S]*routeVoice\(t,\s*6,\s*spec\.stopSec\)[\s\S]*triggerSynthChoke\(t,\s*voiceGain,\s*spec\)/.test(main), 'synthSynth uses selected engine resolver, routeVoice with the synth tail, and mono choke');
