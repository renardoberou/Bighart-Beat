...
assert.deepStrictEqual(serialized.tracks[6].p, { pitch:130, decay:.35, tone:.50, shape:.50 }, 'serialized synth track includes default params');
...
const safeDelayFeedback = serializeProject({ appState, tracks, fx, patterns });
safeDelayFeedback.fx.dly.fb = 0.8;
assert.strictEqual(parseProjectImport(safeDelayFeedback).ok, true, 'delay feedback imports accept the UI-safe 80% ceiling');
assertImportRejectedAt('fx.dly.fb', p => { p.fx.dly.fb = 0.81; }, /fx\.dly\.fb|range|between|0\.8/i);
const unsafeSceneDelayFeedback = serializeProject({ appState, tracks, fx, patterns });
unsafeSceneDelayFeedback.patternFxScenes = [null, { fx: { dly: { fb: 0.81 }, rev: { on: false }, comp: { on: false } } }, null, null];
assertImportRejected(unsafeSceneDelayFeedback, /patternFxScenes\[1\]\.fx\.dly\.fb|range|between|0\.8/i, 'pattern FX scene delay feedback above UI ceiling');
