...
assert.strictEqual(synthVoice.SYNTH_MAX_FREQUENCY_HZ, 2200, 'synth voice exposes the shared 2200 Hz output frequency cap');
assert.strictEqual(synthVoice.SYNTH_ROOT_MAX_HZ, 550, 'synth voice exposes the shared 550 Hz root cap');
...
function assertFiniteBounded(spec, label) {
  ...
  assert(spec.pitchHz >= 40 && spec.pitchHz <= 2200, `${label}: pitchHz bounded to synth hotfix ceiling`);
...
assert.strictEqual(body808.oscType, 'sine', '808 uses a sine sub-oscillator for authentic analog body instead of triangle');
...
assert.strictEqual(resolveSynthVoiceSpec('aphex', { pitch: 900, decay: 0.35, tone: 0.5, shape: 0.5 }).pitchHz, 1062, 'resolver applies engine pitch multiplier before clamping — aphex 1.18× on 900 = 1062 Hz');
