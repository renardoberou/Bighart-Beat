...
assert(/trackId\s*===\s*['"]hihat['"]\s*&&\s*wasOn\s*&&\s*trackIndex\s*===\s*S\.sel/.test(openHihatClickBranch[1]), 'selected active OHH row click is handled before generic OHH off toggle');
...
assert(/hihat-idm-spark-20260522/.test(html), 'cache bust/version marker updated after hihat IDM spark slice');
