...
function assertFiniteSpark(spec, label) {
  ...
  assert(spec.idmSparkGain >= 0 && spec.idmSparkGain <= 0.065 + closedNeedleAccent * 0.007, `${label}: idmSparkGain is bounded/headroom-safe`);
  assert(spec.idmSparkTailSec >= 0.003 - closedNeedleAccent * 0.0005 && spec.idmSparkTailSec <= 0.045, `${label}: idmSparkTailSec is a short mobile-safe crunch tail`);
  assert(spec.idmSparkHz >= 9000 && spec.idmSparkHz <= 18000, `${label}: idmSparkHz stays in high metallic band`);
  assert(spec.idmSparkQ >= 3 && spec.idmSparkQ <= 14 + closedNeedleAccent * 1.5, `${label}: idmSparkQ is focused but bounded`);
}
