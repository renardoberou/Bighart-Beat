...
assert(/Math\.max\([\s\S]*spec\.idmSparkGain\s*>\s*0\.001\s*\?\s*spec\.idmSparkTailSec\s*\+\s*spec\.tailReleaseTau/.test(main), 'hihat tail budget includes IDM spark when enabled');
assert(/if\s*\(\s*spec\.idmSparkGain\s*>\s*0\.001\s*\)\s*\{[\s\S]*const\s+spark\s*=\s*A\.createBufferSource\(\)[\s\S]*spark\.buffer\s*=\s*nz[\s\S]*const\s+sparkFilter\s*=\s*A\.createBiquadFilter\(\)[\s\S]*sparkFilter\.type\s*=\s*'bandpass'[\s\S]*sparkFilter\.frequency\.value\s*=\s*spec\.idmSparkHz[\s\S]*sparkFilter\.Q\.value\s*=\s*spec\.idmSparkQ[\s\S]*sparkGain\.gain\.linearRampToValueAtTime\(clamp\(v \* spec\.idmSparkGain,\s*0,\s*\.065\),\s*t \+ Math\.min\(\.0015,\s*spec\.attackSec\)\)[\s\S]*spark\.connect\(sparkFilter\);\s*sparkFilter\.connect\(sparkGain\);\s*sparkGain\.connect\(choke\);[\s\S]*spark\.stop\(t \+ spec\.idmSparkTailSec \+ spec\.tailReleaseTau\)/.test(main), 'synthHihat wires IDM spark through choke/polish path with bounded gain and stop time');
