...
assert(
  /if\s*\(\s*!audition\s*\)\s*\{\s*triggerSynthChoke\(t,\s*voiceGain,\s*spec\);\s*\}/.test(synthBody),
  'synthSynth only chokes/stores the shared mono voice for non-audition playback'
);
assert(
