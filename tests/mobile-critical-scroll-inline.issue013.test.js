...
assert(/styles\/main\.css\?v=issue014-app-scroll/.test(html), 'index busts the stylesheet cache for the critical mobile scroll fix');
...
assert(/MOBILE-CRITICAL-SCROLL-FIX[\s\S]*\.seq\s*\{[\s\S]*height\s*:\s*clamp\(220px,\s*32svh,\s*300px\)\s*!important[\s\S]*touch-action\s*:\s*pan-x\s+pan-y/.test(html), 'inline fallback lets vertical swipes over the sequencer scroll the page');
assert(/MOBILE-SCROLL-ACCESS-FIX[\s\S]*\.seq\s*\{[\s\S]*height\s*:\s*clamp\(220px,\s*32svh,\s*300px\)[\s\S]*touch-action\s*:\s*pan-x\s+pan-y/.test(css), 'external CSS mobile sequencer also allows vertical page pan');
